...
this.from = function(reportXml, testConfig) {
  ...
  var htmlTitle = testConfig['reportTitle'] == undefined ? reportTitle : '<title>' + testConfig['reportTitle'] + '</title>';
  var htmlReport = '<html><head>' + htmlTitle + reportCss + reportScript + '</head>';
  ...
  // Write report
  var htmlFile = testConfig['outputFile'] == undefined ? 'test-html-report.html' : testConfig['outputFile'];
  fileSystem.writeFileSync(path.join(testOutputPath, '/' + htmlFile), htmlReport);
}
